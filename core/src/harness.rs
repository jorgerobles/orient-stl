...
use crate::scoring::{footprint_area, max_cross_section, score_candidate};
...
fn harness_run() {
    ...
    let configs = [
        WeightCfg { name: "overhang-only (current v1)", w_overhang: 1.0, w_footprint: 0.0, w_cross: 0.0 },
        WeightCfg { name: "footprint-only (naive)", w_overhang: 0.0, w_footprint: 1.0, w_cross: 0.0 },
        WeightCfg { name: "cross-only (peel-force)", w_overhang: 0.0, w_footprint: 0.0, w_cross: 1.0 },
        WeightCfg { name: "equal-weights", w_overhang: 1.0, w_footprint: 1.0, w_cross: 1.0 },
        WeightCfg { name: "resin-biased (cross-heavy)", w_overhang: 0.5, w_footprint: 1.0, w_cross: 2.0 },
        WeightCfg { name: "overhang+footprint (no cross)", w_overhang: 1.0, w_footprint: 1.0, w_cross: 0.0 },
    ];
    let bins = 64usize;
    let crit = 30.0f32;
    ...
    for (path, label) in fixtures {
        ...
        for cfg in &configs {
            ...
            println!(
                "    dir=({:+.3},{:+.3},{:+.3})  over={:.3} foot={:.3} cross={:.3}",
                d[0], d[1], d[2], overs[top.0], fps[top.0], crs[top.0]
            );
