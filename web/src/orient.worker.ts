...
self.onmessage = async (e: MessageEvent<WorkerRequest>) => {
  ...
  try {
    ...
    if (!wasm) { self.postMessage({ type: 'error', message: 'WASM not loaded in worker' } satisfies WorkerMessage); return; }
    ...
    self.postMessage({ type: 'error', message: err?.message ?? String(err) } satisfies WorkerMessage);
  }
};
